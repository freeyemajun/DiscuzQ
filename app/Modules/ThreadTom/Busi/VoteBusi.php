            ...
            $res = array_map(function ($item) {
                ...
                */
                ...
                foreach ($res_subitems as &$val) {
                    ...
                    $val['voteRate'] = 0;
                    if (!empty($res_subitems_sum_votes)) {
                        $val['voteRate'] = round(($val['vote_count']/$res_subitems_sum_votes) * 100, 2).'%';
                    }
    ...
    public function verification()
    {
        ...
        foreach ($input['subitems'] as &$val) {
            ...
            if (in_array($val['content'], $all_content)) {
                $this->outPut(ResponseCode::INVALID_PARAMETER, '投票选项内容不得重复');
            }
            $all_content[] = $val['content'];
            if (mb_strlen($val['content']) > self::SUBITEMS_LENGTH) {
                $this->outPut(ResponseCode::INVALID_PARAMETER, '投票选项最多50个字');
            }
        }
        return $input;
    }
