...
class UserWalletCashReviewController extends DzqAdminController
{
    ...
    public function main()
    {
        ...
        if (!in_array($this->data['cashStatus'], [UserWalletCash::STATUS_REVIEWED, UserWalletCash::STATUS_REVIEW_FAILED])) {
            ...
            $this->outPut(ResponseCode::NET_ERROR, '非法操作');
        }
        ...
        $collection = collect($this->data['ids'])
            ->unique()
            ->map(function ($id) use ($cash_status, &$status_result, $wxpay_mchpay_close, $ip, $db, $log, $log_data,$wxpay_mchpay_close2) {
                $db->beginTransaction();
                //取出待审核数据
                ...
                if (empty($cash_record) || $cash_record->cash_status != UserWalletCash::STATUS_REVIEW) {
                    ...
                    $this->outPut(ResponseCode::INTERNAL_ERROR, '提现审核失败！');
                }
                ...
                if ($wxpay_mchpay_close || $wxpay_mchpay_close2) {        //开通了企业打款
                    try {
                        //检查证书
                        if (!file_exists(storage_path().'/cert/apiclient_cert.pem') || !file_exists(storage_path().'/cert/apiclient_key.pem')) {
                            ...
                            $this->outPut(ResponseCode::INTERNAL_ERROR, '证书不存在，请先上传证书！');
                        }
        ...
        $this->outPut(ResponseCode::SUCCESS, '', $status_result);
    }
