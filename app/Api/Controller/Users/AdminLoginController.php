    ...
    public function main()
    {
        ...
        try {
            ...
            if (empty($e->getMessage())) {
                $this->outPut(ResponseCode::USERNAME_OR_PASSWORD_ERROR);
            }
            if ((int)$e->getMessage() > 0) {
                $this->outPut(ResponseCode::LOGIN_FAILED, '登录失败，您还可以尝试'.(int)$e->getMessage().'次');
            } else {
                $this->outPut(ResponseCode::LOGIN_FAILED, '登录错误次数超出限制');
            }
        }
