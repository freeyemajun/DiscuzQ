...
class LoginController extends AuthBaseController
{
    ...
    public function main()
    {
        ...
        $loginaes =(bool)$this->setting->get('loginaes');
        $key = $this->setting->get('AesKey');
        $iv = $this->setting->get('AesIv');

        if ($loginaes){
            $paramData['username'] = openssl_decrypt(base64_decode($paramData['username']),"AES-128-CBC",$key,true,$iv);
            $paramData['password'] = openssl_decrypt(base64_decode($paramData['password']),"AES-128-CBC",$key,true,$iv);
        }
