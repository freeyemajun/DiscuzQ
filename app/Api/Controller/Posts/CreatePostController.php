...
//        $data['content'] = str_replace($searches, $replaces, $content);
        ...
        $t = Post::query()->where('thread_id', $threadId)->where('is_first', 1)->first();
        $tinfo = Thread::query()->where('id', $threadId)->first();


        if ($this->user->id != $aiuid && (in_array($tinfo->category_id,$arrcid) || ($callai && strpos($t->content, '@'.$aiusername) !== false))){
