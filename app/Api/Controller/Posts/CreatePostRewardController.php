...
class CreatePostRewardController extends DzqController
{
    ...
    public function main()
    {
        ...
        if (!$checkRewards) {
            $this->outPut(ResponseCode::INVALID_PARAMETER, trans('post.post_reward_type_error'));
        }

        if (empty($thread_id)) {
            $this->outPut(ResponseCode::INVALID_PARAMETER, trans('post.post_reward_does_not_have_thread_id'));
        }
        ...
        if (empty($threadReward)) {
            $this->outPut(ResponseCode::INVALID_PARAMETER, trans('post.post_reward_detail_not_found'));
        }

        if ($threadReward['user_id'] !== $actor->id) {
            $this->outPut(ResponseCode::INVALID_PARAMETER, trans('post.post_reward_user_limit'));
        }

        if (Carbon::now() > $threadReward['expired_at']) {
            $this->outPut(ResponseCode::INVALID_PARAMETER, trans('post.post_reward_is_over'));
        }

        if ($threadReward['remain_money'] == 0) {
            $this->outPut(ResponseCode::INVALID_PARAMETER, trans('post.post_reward_the_rewards_is_use_up'));
        }

        if ($rewards <= 0) {
            $this->outPut(ResponseCode::INVALID_PARAMETER, trans('post.post_reward_the_rewards_limit_fail'));
        }

        if ($remain_money < $rewards) {
            $this->outPut(ResponseCode::INVALID_PARAMETER, trans('post.post_reward_not_sufficient_funds'));
        }

        if (!isset($attributes['post_id']) || empty($attributes['post_id'])) {
            $this->outPut(ResponseCode::INVALID_PARAMETER, trans('post.post_reward_does_not_have_post_id'));
        }

        $posts = Post::query()->where(['id' => $attributes['post_id'], 'thread_id' => $thread_id, 'is_comment' => 0])->whereNull('deleted_at')->first();
        if (empty($posts)) {
            $this->outPut(ResponseCode::INVALID_PARAMETER, trans('post.post_reward_post_detail_not_found'));
        }

        if ($posts['user_id'] == $actor->id) {
            $this->outPut(ResponseCode::INVALID_PARAMETER, trans('post.post_reward_post_user_id_limit'));
        }
        ...
        if (empty($threadRewardOrder)) {
            ...
            $this->outPut(ResponseCode::INVALID_PARAMETER, trans('post.post_reward_order_error'));
        }

        // 订单实付金额 !== 悬赏帖金额，不允许进行采纳动作
        if ($threadRewardOrder['amount'] !== $threadReward['money']) {
            $this->outPut(ResponseCode::INVALID_PARAMETER, trans('post.post_reward_not_equal_to_order_price'));
        }
        ...
        try {
            if ($threadRewardOrder['payment_type'] == Order::PAYMENT_TYPE_WALLET) {
                ...
                if ($userWallet->freeze_amount - $rewards < 0) {
                    ...
                    $this->outPut(ResponseCode::INVALID_PARAMETER, trans('post.post_reward_user_wallet_error'));
                } else {
