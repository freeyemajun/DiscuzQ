...
class VoteThreadController extends DzqController
{
    ...
    protected function checkRequestPermissions(UserRepository $userRepo)
    {
        ...
        if ($thread_vote->expired_at < Carbon::now()) {
            $this->outPut(ResponseCode::INVALID_PARAMETER, '投票已过期');
        }
        //判断是单选还是多选
        if ($thread_vote->choice_type == 1 && count($vote['subitemIds']) > 1) {
            $this->outPut(ResponseCode::INVALID_PARAMETER, '该投票是单选，不可多选');
        }
