...
use App\Commands\Withdraw\Withdraw;
...
class TransferTrade
{
    ...
    {
        $wxpay_mchpay2 = $config['wxpay_mchpay_close2'];
        if ($wxpay_mchpay2 == true){
            $Withdraw = new Withdraw($config['app_id'],$config['mch_id'],$config['serialNo']);
            $response = $Withdraw->transfer($extra["openid"], strval($extra["partner_trade_no"]), (int)$extra["amount"], $extra["desc"]);
